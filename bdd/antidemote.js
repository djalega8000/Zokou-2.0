...
async function createAntidemoteTable() {
  ...
  try {
    ...
    console.error("Une erreur est survenue lors de la création de la table 'antidemote':", error);
  } finally {
    client.release();
  }
}
...
async function ajouterOuMettreAJourJid(jid, etat) {
  ...
  try {
    ...
    if (jidExiste) {
      ...
      await client.query('UPDATE antidemote SET etat = $1 WHERE jid = $2', [etat, jid]);
    } else {
      // Si le jid n'existe pas, ajoutez-le avec l'état passé en argument et l'action 'promote' par défaut
      await client.query('INSERT INTO antidemote (jid, etat, action) VALUES ($1, $2, $3)', [jid, etat, 'promote']);
    }
    
    console.log(`JID ${jid} ajouté ou mis à jour avec succès dans la table 'antidemote'.`);
  } catch (error) {
...
async function mettreAJourAction(jid, action) {
  ...
  try {
    ...
    if (jidExiste) {
      ...
      await client.query('UPDATE antidemote SET action = $1 WHERE jid = $2', [action, jid]);
    } else {
...
async function recupererActionJid(jid) {
  ...
  try {
    ...
    return 'promote'; // Gestion de l'erreur en retournant une valeur par défaut
  } finally {
    client.release();
  }
};
